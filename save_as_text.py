...
for movie in all_movies.find_all("div",class_="item"):#找到每一个电影，接下来在所在代码块中找到需要的信息并输出
    ...
    Listdata.append("电影名:{} 海报链接:{} 上映日期:{} 类型:{} 地区:{} 关注的人数:{}\n".format(name,href,date,type,area,lovers)) #加入列表当中

with open("filmdata1.txt", 'w', encoding="utf-8") as text_file:
    text_file.writelines(Listdata)                       #写入数据
